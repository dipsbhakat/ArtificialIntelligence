...
class HeadshotGenerator:
    ...
    def crop_to_headshot(self, image: np.ndarray, padding: float = 0.3) -> np.ndarray:
        ...
        if face_bbox:
            ...
            crop_height = int(height * (1 + padding * 2.5))  # More space below for shoulders
    ...
    def apply_professional_background(self, image: Image.Image, bg_color: str = "white") -> Image.Image:
        """Remove background and apply professional background"""
        try:
            # Remove background
            ...
            if bg_color == "gradient":
                ...
                background = Image.new("RGB", img_no_bg.size, (70, 130, 180))
                gradient = np.linspace(0, 1, img_no_bg.height).reshape(-1, 1)
                gradient = np.repeat(gradient, img_no_bg.width, axis=1)
                gradient_img = Image.fromarray((gradient * 255).astype(np.uint8))
                background.paste(gradient_img, (0, 0))
            else:
                background = Image.new("RGB", img_no_bg.size, bg_colors[bg_color])
...
def main():
    ...
    if uploaded_file is not None:
        ...
        bg_option = st.sidebar.selectbox(
            "Background Style",
            ["white", "light_gray", "blue", "navy", "gradient", "original"]
        )
        
        st.sidebar.subheader("Output Options")
        output_size = st.sidebar.selectbox(
            "Output Size",
            ["400x400 (LinkedIn)", "500x500 (Standard)", "600x600 (High-res)", "Custom"]
        )
        
        if output_size == "Custom":
            width = st.sidebar.number_input("Width", min_value=100, max_value=1000, value=400)
            height = st.sidebar.number_input("Height", min_value=100, max_value=1000, value=400)
            size = (width, height)
        else:
            size_map = {
        ...
        if st.sidebar.button("Generate Headshot", type="primary"):
            with st.spinner("Processing your headshot..."):
                ...
                if bg_option != "original":
                    processed_image = generator.apply_professional_background(
